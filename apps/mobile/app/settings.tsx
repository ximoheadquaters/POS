...
function SettingsContent() {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'taxes' | 'inventory' | 'receipt'>(
    'profile',
  );
  ...
  return (
    <Screen>
      ...
      <View className="border-b border-slate-200/80 bg-white">
        <View className="flex-row items-center justify-between px-4 pt-2 md:hidden">
          <Text className="text-[11px] font-medium text-slate-500">
            Swipe sideways for more settings
          </Text>
          <Feather name="chevrons-right" size={14} color="#64748B" />
        </View>
        <ScrollView
          horizontal
          ...
          {[
            ...
            return (
              ...
              >
                ...
                />
                <Text
                  className={`ml-1.5 text-xs font-bold ${isActive ? 'text-white' : 'text-slate-700'}`}
                >
                  {tab.label}
                </Text>
              </Pressable>
        ...
        {activeTab === 'profile' && (
          <View className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
            <View className="mb-4 flex-row items-center gap-3 border-b border-slate-100 pb-3">
              ...
              <View className="flex-1">
                <Text className="text-base font-bold text-slate-900">
                  Business Identity & Locale
                </Text>
                <Text className="text-xs text-slate-500">
                  Official business name, default currency, and regional timezone
                </Text>
              </View>
            </View>
            ...
            <View className="gap-4">
              ...
              {/* Currency Selector */}
              <View>
                <Text className="mb-1.5 text-xs font-bold tracking-wide text-slate-700">
                  Operating Currency
                </Text>
                <Controller
                  control={form.control}
                      ...
                      {CURRENCY_OPTIONS.map((c) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Text
                              className={`text-xs font-bold ${isSelected ? 'text-emerald-900' : 'text-slate-700'}`}
                            >
                              {c.label}
                            </Text>
                          </Pressable>
        ...
        {activeTab === 'taxes' && (
          <View className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
            <View className="mb-4 flex-row items-center gap-3 border-b border-slate-100 pb-3">
              ...
              <View className="flex-1">
                <Text className="text-base font-bold text-slate-900">
                  Pricing, Taxes & Gross Margins
                </Text>
                <Text className="text-xs text-slate-500">
                  Set checkout Value-Added Tax (VAT) and target profit margin thresholds
                </Text>
              </View>
            </View>
            ...
            <View className="gap-4">
              ...
              <View className="flex-row flex-wrap gap-4">
                <View className="flex-1 min-w-[200px]">
                  ...
                  />
                  <Text className="mt-1 text-[11px] text-slate-500">
                    Recommended target for retail and food service: 25.00%
                  </Text>
                </View>
                ...
                <View className="flex-1 min-w-[200px]">
                  ...
                  />
                  <Text className="mt-1 text-[11px] text-slate-500">
                    Warns staff when selling items below this margin level
                  </Text>
                </View>
              </View>
            </View>
          ...
          <View className="gap-5">
            ...
            <View className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
              <View className="mb-4 flex-row items-center gap-3 border-b border-slate-100 pb-3">
                ...
                <View className="flex-1">
                  <Text className="text-base font-bold text-slate-900">
                    Inventory Policy & Overselling
                  </Text>
                  <Text className="text-xs text-slate-500">
                    Control stock enforcement rules at checkout
                  </Text>
                </View>
              </View>
                  ...
                  <View className="flex-row items-center justify-between rounded-xl bg-amber-50/60 p-4 border border-amber-200/70">
                    <View className="flex-1 pr-4">
                      ...
                      <Text className="mt-1 text-xs text-slate-600 leading-relaxed">
                        When enabled, cashiers can complete sales even if recorded stock is zero.
                        Disable this to prevent selling items out of stock.
                      </Text>
                    </View>
                    <Switch
                      disabled={!editable}
            ...
            <View className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
              <View className="mb-4 flex-row items-center gap-3 border-b border-slate-100 pb-3">
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-xs text-slate-500">
                    Select payment channels available to cashiers at checkout
                  </Text>
                </View>
              </View>
                          ...
                          if (isChecked) {
                            if (currentMethods.length <= 1) {
                              showAlert({
                                ...
                                message:
                                  'At least one payment method must remain enabled for POS checkout.',
                                type: 'warning',
                              });
                              return;
                            }
                        ...
                        return (
                          ...
                          >
                            <View className="flex-row items-center gap-3">
                              <View
                                className={`h-8 w-8 items-center justify-center rounded-lg ${isChecked ? 'bg-white shadow-2xs' : 'bg-slate-100'}`}
                              >
                                <Feather
                              ...
                              <Text
                                className={`text-sm font-bold ${isChecked ? cfg.text : 'text-slate-700'}`}
                              >
                                {cfg.label}
                              </Text>
                            </View>
                            ...
                            <View
                              className={`h-6 w-6 items-center justify-center rounded-md border ${isChecked ? 'border-emerald-600 bg-emerald-600' : 'border-slate-300 bg-white'}`}
                            >
                              {isChecked ? (
            ...
            <View className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
              <View className="mb-4 flex-row items-center gap-3 border-b border-slate-100 pb-3">
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-xs text-slate-500">
                    Custom message printed at the top and bottom of customer receipts
                  </Text>
                </View>
              </View>
          ...
          <View className="mt-4 flex-row items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <View className="flex-1 pr-3">
              <Text className="text-xs font-bold text-slate-900">
                Unsaved Business Configuration
              </Text>
              <Text className="text-[11px] text-slate-500">
                Changes will take effect immediately across all POS registers.
              </Text>
            </View>
            <Button
